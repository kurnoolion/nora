...
- **Global taxonomy is fingerprint-cached** (D-DRAFT-9): `run_taxonomy` is a global stage — it derives ONE union feature set over every cell's trees (rglob), so cross-MNO comparison shares a feature space. Because it's LLM-driven, expensive, and run-to-run non-deterministic, derivation is gated on a **corpus fingerprint** (hash of the contributing tree set, path + content; written to `out/taxonomy/.corpus_fingerprint`): an unchanged tree set reuses the cached `taxonomy.json` (`stats.source == "cache"`), a changed set re-derives (`"derived"`); `--force` busts the cache. Per-MNO corpus-overview files (`$NORA_TAXONOMY_OVERVIEW_DIR/corpus_overview_*.txt`, strand sira-enrichment-pe) are prompt inputs and are hashed into the fingerprint too — adding/editing/removing one re-derives without `--force`. Extraction runs at `temperature=0` (already in `FeatureExtractor`) for reproducibility. So an incremental cell add doesn't silently re-derive (and re-shuffle) the whole corpus's features unless the trees actually changed.
- **Taxonomy: plan-unit extraction, newest release wins, per-plan fail-soft + resume** (strand sira-enrichment-pe): the unit of extraction is a **plan**, not a tree file. `_select_newest_trees` first dedupes files to the newest instance of each `(MNO, plan_id)` — "newest" = chronologically greatest release directory name via `_release_sort_key` (MMMYYYY → `YYYYMM`, e.g. `Jul2026` → `202607`; non-MMMYYYY names fall back to lexicographic), ties broken by file name. Then each tree is split via `taxonomy.split_tree_by_plan` (chapter-per-plan docs → one subtree per plan) and **units** are deduped by `(MNO, plan_id)` with the same release ordering — this second pass is what catches multi-plan docs across releases, whose empty tree-level plan_id makes them invisible to file-level selection. Rationale: the flat `out/taxonomy/<plan_id>_features.json` layout and the downstream per-plan lookup key on `plan_id` alone; older copies would only be LLM-extracted and then overwritten. The fingerprint hashes the *selected file* set, so edits to superseded trees don't bust the cache. Extraction is per-plan fail-soft: an LLM error or unparseable response (`LLMParseError`) marks that unit `failed` in `out/taxonomy/extraction_state.json` (keys: `<tree-relpath>` for single-plan trees, `<tree-relpath>#<plan_id>` for split ones; written after every unit — survives hard kills) and the run continues; unchanged `ok` units (same tree hash + same overview-prompt hash) are reused on re-run, and `failed` units retry automatically — re-running the same command *is* the retry mechanism. A partial run consolidates what succeeded, returns `WARN` (`TAX-W004`), and does **not** stamp the fingerprint, so a degraded run can never cache-lock itself; all-units-failed is a `FAIL`. Stale `*_features.json` with no matching selected plan (including a multi-plan doc's old empty-prefix `_features.json`) are deleted on re-derive.
...
- `lane_bounds` — function — pub — Map a --lane alias to its (start, end) stage names (PIPELINE_LANES).
- `logger` — constant — pub
- `main` — function — pub
...
- `_RELEASE_RE` — constant — internal
- `_cell_extract_hints` — function — internal — Per-cell PDF extractor hints from each in-scope cell's bound profile
...
- `_release_sort_key` — function — internal — Chronological sort key for MMMYYYY release names.
- `_select_newest_trees` — function — internal — Newest instance of each (MNO, plan_id) across releases.
...
- `run_vectorstore` — function — pub — Build one vector store PER CELL (D-DRAFT-11).
<!-- END:STRUCTURE -->
