...
- **Taxonomy: newest release wins, per-doc fail-soft + resume** (strand sira-enrichment-pe): before extraction, `_select_newest_trees` dedupes trees to the newest instance of each `(MNO, plan_id)` — "newest" = chronologically greatest release directory name via `_release_sort_key` (MMMYYYY → `YYYYMM`, e.g. `Jul2026` → `202607`; non-MMMYYYY names fall back to lexicographic), ties broken by file name — because the flat `out/taxonomy/<plan_id>_features.json` layout and the downstream per-plan lookup key on `plan_id` alone; older copies would only be LLM-extracted and then overwritten. The fingerprint hashes the *selected* set, so edits to superseded trees don't bust the cache. Extraction is per-doc fail-soft: an LLM error or unparseable response (`LLMParseError`) marks that doc `failed` in `out/taxonomy/extraction_state.json` (written after every doc — survives hard kills) and the run continues; unchanged `ok` docs (same tree hash + same overview-prompt hash) are reused on re-run, and `failed` docs retry automatically — re-running the same command *is* the retry mechanism. A partial run consolidates what succeeded, returns `WARN` (`TAX-W004`), and does **not** stamp the fingerprint, so a degraded run can never cache-lock itself; all-docs-failed is a `FAIL`. Stale `*_features.json` with no matching selected plan are deleted on re-derive.
