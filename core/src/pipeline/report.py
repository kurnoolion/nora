...
def _format_stats_compact(stage: str, stats: dict) -> str:
    ...
    formatters = {
        ...
        "parse": lambda s: f"req={s.get('reqs',0)} dep={s.get('max_depth',0)} docs={s.get('docs',0)} toc={s.get('toc',0)} struck={s.get('struck',0)} cascade={s.get('cascade',0)} revhist={s.get('revhist',0)} defs={s.get('defs',0)}",
