...
def _cell_extract_hints(ctx: PipelineContext) -> dict[tuple[str, str], dict]:
    ...
    if not override:
        corr = ctx.correction("profile.json")
        if corr:
            override = str(corr)
    env_dir = ctx.stage_output("extract").parent.parent
    try:
        ...
        try:
            ...
            out[(cell.mno.upper(), cell.release)] = {
                ...
                "enable_table_anchored_extraction": (
                    prof.enable_table_anchored_extraction),
            }
        except Exception:
            continue
    return out
...
def run_extract(ctx: PipelineContext) -> StageResult:
    ...
    for f in files:
        ...
        try:
            ...
            # Provider tables carry the flat headers/rows only when the anchoring
            # path needs them — otherwise HTML is the single structured form.
            grid = hints.get("enable_table_anchored_extraction", True)
            ir = extract_document(
