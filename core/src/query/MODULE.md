...
**Public surface**
- Entry point: `QueryPipeline(graph, embedder, store, analyzer=None, synthesizer=None, rewriter=None, reranker=None, top_k=10, max_depth=None, max_context_chars=30000, enable_bm25=True, max_distance_threshold=None, enable_grouping=False, top_k_cap=None, cell_stores=None)` (pipeline.py) — `query(query_text, verbose=False, pinned_chunk_ids=None, *, synthesizer=None) -> QueryResponse`. `cell_stores` (D-DRAFT-11, strand `multi-mno-nora`) is a `{(mno, release): store}` map (from `vectorstore.load_cell_stores`) giving one retriever per cell; query-time `_select_cells` routes by the resolved `(mno, release)` scope, then **single cell → its retriever directly; multiple cells → per-cell retrieve, merge, rerank-once** (merge-then-rerank). A single flat `store` normalizes to `{FLAT_CELL: store}` so existing callers are unchanged. Constructor knobs added since the original 6-stage shape: `max_distance_threshold` [D-047], `enable_grouping` [D-049], `top_k_cap` (hard ceiling applied AFTER per-type widening). The `pinned_chunk_ids` parameter on `query()` skips Stages 2–4.7 and synthesizes only from the named chunks — used by the disambiguation user-pick UX (D-049 Step 3c). The keyword-only `synthesizer` parameter overrides Stage 6 for that one call, so a caller can vary the LLM per query (e.g. a per-question reasoning level from the Ask page) while reusing the cached pipeline.
...
- Schema (schema.py):
  ...
  - Per-stage dataclasses: `QueryIntent`, `MNOScope`, `ScopedQuery`, `CandidateNode`, `CandidateSet`, `RetrievedChunk`, `StandardsContext`, `ChunkContext`, `AssembledContext`, `Citation`, `QueryResponse` (gained `disambiguation_required`, `groups`, `assembled_context`, `citation_audit`, `timings_ms` fields), `ChunkGroup` (Stage 4.7 output [D-049]), `SentenceAudit` + `CitationAudit` (Stage 6.5 output [D-052]).
  - Stage timing vocabulary: `STAGE_ORDER` — `(slug, display_label, band)` triples in pipeline execution order; `BAND_ORDER` — the coarse phases (`prep`, `retrieval`, `synthesis`, `post`, `unaccounted`). The bands exist so this module's ten stages and the SIRA service's four (`expand` / `search` / `rerank` / `synth`) can be read side by side in one UI, since the stage names are not comparable across lanes but the bands are.
  - `QueryResponse.timings_ms: dict[str, int]` — per-stage wall-clock milliseconds, keyed by `STAGE_ORDER` slugs, plus `total_ms` for the whole `query()` call. `QueryResponse.unaccounted_ms` is `total_ms` minus the sum of the stages, clamped at 0.
...
- `query(synthesizer=...)` applies to that call only and is never stored on the instance — one request's LLM settings cannot leak into the next. This is what makes a per-question reasoning level safe while the pipeline is shared across concurrent queries.
- **A stage that did not run has NO key in `timings_ms`.** Stages 3, 3.5, 4.5 and 4.7 are all conditional, and a `0` would render a bypassed stage as instantaneous rather than skipped. Consumers must treat a missing key as "did not run"; only an absent key carries that meaning. Every one of `query()`'s five return paths populates `timings_ms` (including the pinned-chunks, threshold-not-found and disambiguation short-circuits), so the field is never empty on a successful call.
- **`timings_ms` does not have to sum to `total_ms`, and `total_ms` does not cover pipeline construction.** The un-instrumented remainder is real; surface it rather than distributing it across the stages. Web callers use the route's wall clock as the denominator so cold-start construction shows up too — see `web.timeline.build_timeline`.
...
- `MockQueryAnalyzer` — class — pub — Keyword-based query analyzer (no LLM required).
  ...
  - `_extract_mnos` — method — internal — Extract MNO references (token-boundary match — see
...
- `_MNO_ALIAS_PATTERNS` — constant — internal
...
- `logger` — constant — pub
- `make_query_analyzer` — function — pub — Select the query analyzer by NORA's standard rule.

`bm25_index.py`
- `BM25Index` — class — pub — Wraps `rank_bm25.BM25Okapi` with the chunk corpus + telecom
  - `__init__` — constructor — internal
  - `chunk_metadata` — method — pub
  - `chunk_text` — method — pub — Retrieve a chunk's text by id. Used by hybrid retrieval to
  - `from_store` — classmethod — pub — Build a BM25Index from a `VectorStoreProvider.get_all()`
...
- `_path_of` — function — internal — Read the chunk's hierarchy_path as a tuple. Returns () for
...
- `QueryPipeline` — class — pub — End-to-end query pipeline.
  ...
  - `_select_cells` — method — internal — Target cell keys for a query (D-DRAFT-11).
  - `query` — method — pub — Run the full query pipeline.
- `_DEFAULT_RERANK_ENABLED` — constant — internal
...
- `RAGRetriever` — class — pub — Retrieves and ranks requirement chunks by similarity.
  ...
  - `_exclude_markers` — staticmethod — internal — Append `chunk_role != marker` to an existing where clause.
...
`reranker.py`
- `CrossEncoderReranker` — class — pub — Wraps `sentence_transformers.CrossEncoder` to score (query,
  - `__init__` — constructor — internal
...
- `MockReranker` — class — pub — Deterministic no-op reranker — returns input chunks as-is.
  - `rerank` — method — pub
- `OllamaReranker` — class — pub — Cross-encoder reranker that scores (query, document) pairs via
  - `__init__` — constructor — internal
  - `_cosine` — staticmethod — internal — Cosine similarity. Returns 0.0 when either vector is empty
  - `_embed_raw` — method — internal — One Ollama call; return the raw vector (any dimensionality).
  - `_probe_endpoint_and_mode` — method — internal — Discover which Ollama endpoint + scoring mode this model
  - `_rerank_by_score` — method — internal — True cross-encoder mode: one call per (query, passage) pair;
  - `_rerank_by_similarity` — method — internal — Embedding-similarity mode: embed query once, embed each
  - `_truncate` — method — internal
  - `available` — property — pub
  - `rerank` — method — pub — Score each chunk against the query; return descending order.
- `OpenAIRerankChat` — class — pub — Reranker that scores (query, document) pairs via an OpenAI-
  - `__init__` — constructor — internal
  - `_format_batch_prompt` — method — internal — Build a batch-scoring prompt for ``docs=[(local_id, text), ...]``.
  - `_parse_batch_response` — staticmethod — internal — Extract ``{id: score}`` from a batch rerank response. Missing /
  - `_score_batch` — method — internal — Score one batch in a single HTTP call. Returns {local_id: score};
  - `_score_one` — method — internal — Score one (query, doc) pair. Returns 0 on any failure so
  - `_truncate` — method — internal
  - `rerank` — method — pub
- `OpenAIRerankDedicated` — class — pub — Reranker that calls an OpenAI-compatible ``/v1/rerank`` endpoint
  - `__init__` — constructor — internal
  - `_truncate` — method — internal
  - `rerank` — method — pub
- `Reranker` — class — pub — Protocol for rerankers.
  - `rerank` — method — pub
- `TEIReranker` — class — pub — Reranker that calls a HuggingFace TEI (Text Embeddings Inference)
  - `__init__` — constructor — internal
  - `_score_batch` — method — internal — One /rerank call. Returns {local_idx: score} on success, ``None``
  - `_truncate` — method — internal
  - `rerank` — method — pub
...
- `_ollama_embed` — function — internal — Call /api/embeddings directly. Returns (vec, None) or (None, error_msg).
- `_section_embedding_model` — function — internal — Direct API probe of the embedding model — proves identical bytes.
- `_section_inspect_req` — function — internal — Vectorstore + BM25 view of one chunk; optionally score vs query.
...
- `cmd_inspect_req` — function — pub — Load store + BM25, dump one chunk's view.
- `logger` — constant — pub
- `main` — function — pub
...
- `CitationAudit` — dataclass — pub — Full audit of a synthesized answer. Output of Stage 6.5.
...
- `ScopedQuery` — dataclass — pub — Query with resolved MNO/release scope.
  - `to_dict` — method — pub
- `SentenceAudit` — dataclass — pub — One sentence's audit result. Output of post-synthesis Stage 6.5.
