...
- Shared requirement-tree loader (req_tree.py, strand golden-eval): `load_tree_flat` / `build_tree_hierarchy` / `load_tree` / `find_tree` / cell-and-doc discovery (`list_cells` / `list_docs`), picker cascade data (`plans_for_mno` — plan → releases union, `reqs_for_plan`), and `find_req` (corpus-wide id lookup driving direct-entry validation + auto-qualification) — lifted from `routes/req_browser.py`'s module-private helpers; used by both req_browser and the Eval Studio picker. Per-cell layout first, legacy flat fallback.
