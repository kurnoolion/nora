...
- Eval Studio (routes/golden_eval.py, strand golden-eval — design; code pending):
  ...
  - Direct-paste entry — known req_ids validated against the store at save time; an id resolving in exactly one cell is auto-qualified, ambiguous ids prompt, unresolvable ids block `stage1-ready`.
  ...
  - Team-mode gate admits `/eval-studio` (domain experts are the gated team); destructive ops (sample delete) admin-gated.
- Shared requirement-tree loader (req_tree.py, strand golden-eval — design; code pending): `load_tree_flat` / `build_tree_hierarchy` / cell-and-doc discovery lifted from `routes/req_browser.py`'s module-private helpers; used by both req_browser and the Eval Studio picker.
