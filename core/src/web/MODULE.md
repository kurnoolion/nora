...
- Markdown rendering (markdown_render.py):
  ...
  - `render_markdown_bubbles(text, req_ids) -> Markup` — `render_markdown` plus req-ID bubbles (strand req-id-bubbles): each id in `req_ids` that appears verbatim in the rendered prose becomes a click-to-expand badge that HTMX-loads the requirement from `GET /api/req/{req_id}`. Registered as the `md_bubbles` Jinja filter. Substitution skips tag interiors and the contents of `<a>` / `<pre>`, so markup and fenced blocks cannot be corrupted; inline `<code>` IS bubbled (LLMs routinely backtick req IDs). Empty / missing `req_ids` degrades to plain `render_markdown` output.
