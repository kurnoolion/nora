...
**Invariants**
- The roster itself is deliberately NOT committed in this repo: a useful roster names real endpoints, and committed configs here stay fictional. The DEPLOYED roster is committed in the internal repo at `customizations/llm/llm.json` (D-248) — same convention as `customizations/mappings/` and `customizations/prompts/` (D-062: committed internally, kept off github.com by the work-PC pre-push guard) — baked into the image and selected via `NORA_LLM_CONFIG=/app/customizations/llm/llm.json`; a copy-from example (`customizations/config/llm.json.example`) also lives internally. `test_ask_reasoning.py::TestExampleConfigStaysValid` validates whichever of the two is present (the committed roster preferred) — it must parse, document every `LLMProviderEntry` field (generative over the dataclass), and contain no literal API key — and skips in clones with neither file; only value-free properties are asserted, because pinning roster values in a committed test would put them back in this repo.
...
- `reasoning_control` names the **wire form** — a separate question from whether the endpoint honours anything at all. `"reasoning_effort"` sends the OpenAI-standard field; `"chat_template_kwargs"` sends `{"enable_thinking": false}`; `"none"` says the endpoint honours neither. The declared-not-detected argument above applies here with more force, not less: the two endpoints on the internal roster each **reject the other's form** (one 400s on `extra_forbidden`, the other 400s on `literal_error` because its enum is low|medium|high and `low` still thinks), so there is no wire form that works everywhere and no probe that can choose safely. `LLMProviderEntry.reasoning_mechanism` resolves the two declarations: an explicit `reasoning_control` wins and implies support, `"none"` means no control, and an unset value falls back to `"reasoning_effort"` when the legacy boolean is true — the only mechanism such a roster could have meant, since it was the only form this code ever sent. An unrecognised value is dropped at load with a warning (`REASONING_CONTROLS`), so a typo degrades to "no control" instead of putting an unknown key on the wire.
