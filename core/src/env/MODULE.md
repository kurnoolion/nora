...
**Invariants**
- The roster example — the copy-from reference for the whole schema — is deliberately NOT committed in this repo: a useful example names real endpoints, and committed examples here stay fictional. It lives in the internal repo at `customizations/config/llm.json.example` — same convention as `customizations/mappings/` and `customizations/prompts/` (D-062: committed internally, kept off github.com by the work-PC pre-push guard) — and is never read by the app. `test_ask_reasoning.py::TestExampleConfigStaysValid` validates it where present — it must parse, document every `LLMProviderEntry` field (generative over the dataclass), and contain no literal API key — and skips in clones without the file; only value-free properties are asserted, because pinning example values in a committed test would put them back in this repo.
