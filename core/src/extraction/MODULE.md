...
- **Background-band filter** (D-183, strand `mno-b-tables`): before `find_tables()`, fill-only stroke-less rects/curves covering ≥50% of the page area (`_BG_RECT_PAGE_FRAC = 0.5`) are dropped from the pdfplumber page — decorative page bands otherwise join the "lines"-strategy table lattice, inflate the detected bbox, and the committed-bbox text suppression swallows every body paragraph on the page. Identity no-op on pages without such objects; the "curve" arm covers producers that paint the band as a closed path instead of a `re` rect. Synthetic-PDF tests pin pdfplumber's inflate/hug behavior so an upgrade that changes lattice construction surfaces as a test failure.
