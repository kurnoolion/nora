...
- `ContentBlock` — single block with type-specific fields (text/level, headers+rows, image_path, object_type+extracted_content) plus per-run formatting: `runs: list[TextRun]` (paragraphs/headings); `header_runs: list[list[TextRun]]` and `row_runs: list[list[list[TextRun]]]` (tables); `merged_cells: list[MergedCell]` (tables — empty when no merges); `lines: list[str]` (PDF paragraph/heading source-line boundaries — preserves the per-line split that `text` flattens with spaces so a heading/title line stays distinct from the body line(s) beneath it; invariant `" ".join(lines) == text`; empty for legacy IRs and non-PDF extractors). Helpers: `live_text()` (concat unstruck runs); `row_all_struck(i)`, `header_all_struck()`, `cell_live_text(r, c)`, `header_live_text(c)` for tables.
