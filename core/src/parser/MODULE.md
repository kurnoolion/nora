...
- `build_context_string(parent_section, section_index, mode) -> str` (structural_parser.py) [D-DRAFT-5] — shared pure helper that assembles a requirement's enclosing-section context from the tree's section nodes, per `mode` (`"none"` / `"path"` / `"path_and_content"`). Called by consumers (SIRA adapter, chunk builder) at emit time, not by the parser. `RequirementTree.build_context` stamps the profile's mode for consumers to read.
