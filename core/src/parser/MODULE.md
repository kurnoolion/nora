...
- `Requirement` — single requirement node: `req_id`, `plan_id` (D-DRAFT-1 — plan extracted per-req from `req_id`), `section_number`, `title`, `parent_req_id`, `parent_section`, `hierarchy_path`, `zone_type`, `priority` (FR-31), `applicability: list[str]` (FR-32 [D-030]), `text`, `tables`, `images`, `children`, `cross_references`
