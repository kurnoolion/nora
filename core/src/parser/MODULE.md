...
- `GenericStructuralParser` — class — pub — Profile-driven structural parser for requirement documents.
  ...
  - `_apply_content_end_marker` — method — internal — Truncate each requirement's text at `profile.content_end_marker` and drop trailing tables/images (D-DRAFT-13).
  ...
  - `_drop_excluded_sections` — method — internal — Drop sections whose title matches `profile.exclude_section_pattern` plus descendants (D-DRAFT-13).
  - `_drop_glossary_subtree` — method — internal — Remove the glossary section and all of its descendants (embed_glossary=False).
