...
- `Requirement` — single requirement node: `req_id`, `plan_id` (D-DRAFT-1 — plan extracted per-req from `req_id`), `section_number`, `title`, `parent_req_id`, `parent_section`, `hierarchy_path`, `zone_type`, `priority` (FR-31), `applicability: list[str]` (FR-32 [D-030]), `text`, `context` (D-DRAFT-5 — ancestor-section context field; **left empty in the parsed tree** — consumers assemble it at emit time via `build_context_string` per the tree's `build_context` mode, to avoid duplicating section text per-requirement), `tables`, `images`, `children`, `cross_references`, `is_requirement: bool` (mno-c-ingestion — True iff `req_id` matches `requirement_id.requirement_type_pattern`; distinguishes actual requirements from structural sections when a broadened `pattern` captures section ids too; serialized to tree JSON, back-compat load default `bool(req_id)`)
- `guard_req_id_capture(raw, pattern_re, side="start") -> (guarded, action)` (structural_parser.py, strand `id-precision`) — module-level over-capture guard for raw pattern-matched req_ids; action is `"pass" | "recovered" | "rejected"`. Shared with the extract-side recall checker (`sandbox/verify_req_recall.py`) so both sides apply identical semantics — a checker that over-captures in agreement with the parser hides the corruption.
- `build_context_string(parent_section, section_index, mode, max_chars=0) -> str` (structural_parser.py) [D-DRAFT-5] — shared pure helper that assembles a requirement's enclosing-section context from the tree's section nodes, per `mode` (`"none"` / `"path"` / `"path_and_content"`). `max_chars` (0 = uncapped) caps **each ancestor's body** in `path_and_content` so a huge ancestor section can't blow a chunk past the embedder input limit. Called by consumers (SIRA adapter, chunk builder) at emit time, not by the parser. `RequirementTree.build_context` / `build_context_max_chars` stamp the profile's mode + cap for consumers to read.
- `TableData`, `ImageRef`, `StandardsRef`, `CrossReferences` — nested types; `TableData.html` (mno-c-ingestion — lossless layout-provider table HTML; when set and table-anchored extraction is off, the flat `headers`/`rows` grid is dropped and HTML is the sole table content)
...
- **Every Requirement is anchored from exactly one source block** — one of three sources, selected by the profile: a **paragraph anchor** (heading-style block, or a small-font standalone-ID paragraph), a **table-cell anchor** (a row whose cell matches the profile's `requirement_id.pattern`), or a **leading-id body anchor** (a body paragraph whose text *begins* with `requirement_id.pattern`, active only when `requirement_id.detection_mode == "leading_id_body"` — D-DRAFT-2, strand `multi-mno-nora`). Paragraph anchors win on duplicate `req_id`s; the table-anchor pass dedups against the paragraph-anchor set.
- **Per-requirement plan** (`Requirement.plan_id`, D-DRAFT-1): the plan is extracted per requirement from its `req_id` (profile `requirement_id.components`), so one document may carry multiple plans; falls back to the tree-level `plan_id` when a req has no extractable plan. `RequirementTree.plan_id` stays the document's *primary* plan and may be empty for a genuinely multi-plan document. `RequirementTree.detection_mode` mirrors the profile's mode and is stamped on the tree so consumers can distinguish a heading-model table-anchored req from a leading-id req (identical in the serialized tree). **Per-req-plan grouping is consumer behavior gated on `detection_mode`**: leading-id corpora group by `Requirement.plan_id`; `"heading"` corpora (MNO-A) keep one-plan-per-document — unchanged.
- **Requirement detection mode** (`requirement_id.detection_mode`, D-DRAFT-2): `"heading"` (default) — requirements are heading-anchored, the historical/MNO-A shape, unchanged. `"leading_id_body"` — section/subsection headings are *non-requirement context* (no req_id) and each requirement is a flat body paragraph leading with its id; the enclosing heading provides `parent_section` / `hierarchy_path`. For corpora whose sections carry no req_ids (e.g. a single PDF of `<PREFIX>-<PLAN>-<DIGITS>` requirements). Selected in the profile only — no parser code path is MNO-specific (D-003 / FR-3).
- Table-anchored **and leading-id body** Requirements have `section_number=""` by design — they're addressed by `req_id` and linked to their owning paragraph-anchored section via `parent_section` / `parent_req_id`. Consumers must not assume `section_number` is non-empty. In `"leading_id_body"` mode the parent heading itself may carry an **empty `req_id`** (it's structural context) — consumers that group by heading `req_id` must tolerate this.
- **Leading-id title/body split** (D-DRAFT-5, `leading_id_body` only): a leading-id requirement's `title` is the header line after the req_id and `text` is the body, split on `ContentBlock.lines[0]` vs the rest (D-DRAFT-3 preserved boundaries; falls back to empty title + whole-block text when a block has no `lines`). A multi-line title's overflow falls into `text` (no per-span signal to delimit it — deferred). No-op outside `leading_id_body`.
- **Leading-id TABLE/IMAGE attachment** (D-184, strand `mno-b-tables`): in `leading_id_body` mode, TABLE and IMAGE blocks attach to `current_leading_req or current_section` — a table/image following a requirement paragraph is that requirement's content, mirroring the paragraph-continuation branch — with a fresh-heading guard (a table/image arriving immediately after a heading attaches to the heading's section and clears the stale requirement cursor). Deferred table-anchored extraction keeps the SECTION as parent even in this mode; `previous_block_was_heading` is read-only in these branches, so heading-continuation merging is identical in both modes. Heading mode unchanged.
- **Context is consumer-assembled, not tree-materialized** (D-DRAFT-5, generic): the parser leaves `Requirement.context` empty in the tree (materializing each requirement's full ancestor-section content would bloat the single all-plans tree). Instead the profile carries `build_context` (`"none" | "path" | "path_and_content"`, stamped onto `RequirementTree.build_context`) and the module function `build_context_string(parent_section, section_index, mode)` assembles the enclosing section chain (numbers + titles for `path`, plus each ancestor's body for `path_and_content`, top-down — e.g. `5 Bands > 5.1 Frequency > 5.1.2 LTE`). Downstream consumers (SIRA BEIR adapter, NORA chunk builder) call it at emit time, baking context into the derived retrieval rows where self-contained text is wanted. Generic across detection modes — `bs_d7a2c81f` (heading) uses `path`, `bs_5114ac92` (leading-id) uses `path_and_content`.
...
- **Content-start cutoff** (D-DRAFT-4, opt-in via `profile.content_start_section`): a pre-pass finds the first **real heading** — gated on heading-level font (`_is_heading_font`: bold or size > `body_text.font_size_max`), so a same-numbered TOC entry (body-size) does **not** trigger it — whose top-level section number equals `content_start_section`, and folds `that index − 1` into `_frontmatter_cutoff`. Drops the front page / TOC / intro chapters in one positive anchor (categorized `front_matter`), for corpora whose front matter isn't reliably detectable negatively. Independent of the `toc_detection.style_pattern` gate; empty `content_start_section` (default) = no-op, so existing corpora (MNO-A) are unaffected.
...
- **Profile-driven section exclusion** (D-DRAFT-13, strand `multi-mno-nora`): when `profile.exclude_section_pattern` is non-empty, the parser drops every section whose **title** matches (plus its descendants) from `RequirementTree.requirements` — they never become Requirements or RAG chunks. For non-normative sections that carry a heading (sometimes a trailing req_id) but aren't requirements: bibliographies (`REFERENCES`), requirement→test-case traceability matrices, etc. Runs **after** `reference_list_section_pattern` extraction, so a REFERENCES section still populates `reference_list_map` before being dropped from chunks. A matched section with a `section_number` drops its whole subtree (shared `_drop_section_subtree` helper, also used by the glossary drop); a matched node without one drops just itself. A malformed regex is logged and treated as no-op. Empty pattern (default) = no-op, so other corpora are unaffected.
- **Trailing-appendix truncation** (D-DRAFT-13): `profile.content_end_marker` (regex per body line) handles a non-normative appendix the parser glues onto the *last real requirement* rather than a separate section — e.g. a requirement→test-case traceability matrix + test-case tables that begin on their own line at the end of the document (so `exclude_section_pattern` can't catch them, since there's no heading). For a requirement whose body has a matching line, the parser truncates the text **before** that line and clears the requirement's `tables`/`images` (the appendix's matrices). Symmetric to `content_start_section` (D-DRAFT-4) which cuts the front. Empty (default) = no-op.
...
- **Req-id over-capture guard** (strand `id-precision`): every raw pattern match becomes a req_id only through ``guard_req_id_capture`` — a permissive plan-token class under a one-sided anchor can run past the real id into prose (any prose ending in ``-<digits>`` completes the match) and whitespace canonicalization welds the capture into one silently corrupted token; the hazard is class × anchor (field-validated: 354 corrupted served rows under a leading anchor, 1/cell under the end-anchored fused-heading rescue). Guard semantics are **bounds-first** (field-corrected — the first deployment recovered/rejected on every whitespace-bearing capture with a 3-word bound, truncating legitimate multi-word ids with interior ``-<digits>`` segments and rejecting ~220 legitimate ids per cell on a 5-word-plan-token corpus): an in-bound capture (≤ ``requirement_id.guard_max_words`` whitespace-separated words — profile-tuned corpus inventory, module default 6 — and ≤64 chars) passes UNTOUCHED, never recovered, never rejected — with ONE bounds-independent exception, the **containment trigger**: a capture whose proper whitespace-bounded prefix slice AND suffix slice each fullmatch the pattern holds ≥2 complete ids (the ``id + prose + id`` weld, which can sit inside the legitimate word envelope — field-measured: no word bound separates it, 0 false positives across a full cell inventory since only one edge of a legitimate id carries the literal prefix) and recovers from the anchored side; an over-bound capture gets weld recovery — the shortest whitespace-bounded, itself-in-bound slice from the capture's ANCHORED side that fullmatches the pattern (``side="start"`` for start-anchored/unanchored matches, ``side="end"`` for the end-anchored trailing rescue and TOC tail-peel — prefix recovery there would resurrect a mid-text citation) — else a loud no-id skip. Counters: ``ParseStats.req_id_over_captures_recovered`` / ``req_id_captures_rejected``; logs carry the recovered id and structural counts only, never the raw tail (may contain document prose). Applied at every capture seam: heading anchors (all modes), the fused-heading rescues, body/table scavenge (``_find_req_ids``), leading-id body anchors, the small-font stamp, announcement paragraphs, and the TOC body peel. The extract-side recall checker reads the SAME profile knob — a divergent bound would diff clean parses as MISSING.
- **Labeled-id extraction** (``RequirementIdPattern.id_label_pattern``, mno-c-ingestion): when set, it is the authoritative req_id source — the node's OWN id is captured ONLY from the labeled marker (e.g. trailing ``(Priority) ID: <id>``, id in group 1), overriding ``anchor``, and the same label requirement gates all three heading-mode id paths (heading inline, standalone small-font id block, body-text scavenge). A bare req-id anywhere else — a release-notes citation, a cross-reference — is a *reference*, never mistaken for a definition. Headings with no labeled id get no req_id. Empty (default) = anchor behavior, other corpora unchanged.
- **Requirement-vs-section discriminator** (``RequirementIdPattern.requirement_type_pattern``, mno-c-ingestion): post-link pass sets ``Requirement.is_requirement = bool(req_id matches the narrow pattern)``; empty pattern = legacy "req_id present ⇒ requirement". Lets a broadened ``pattern`` capture structural-section ids (for parent linking / context) without downstream treating sections as requirements; carried into chunk metadata by the vectorstore.
- **Priority scoping** (``heading_detection.priority_requires_req_id``): when set, the priority marker is mined only from headings that carry a req_id; a marker-shaped token in a structural heading's title is neither read as priority nor stripped. ``heading_detection.title_strip_pattern`` then strips the trailing id marker from every heading title (requirement and structural) so titles stay clean.
...
- **Req-ID anchor sources** (profile-selected): paragraph anchors (heading-block, pending-id resolution, or inline body-text id), table-cell anchors (column-1-of-row first; all-cells fallback; one anchor per row max), and — in `detection_mode == "leading_id_body"` (D-DRAFT-2) — leading-id body anchors (a body block whose text *opens* with the req_id pattern; an `^`-anchored match so a mid-text mention is a cross-reference, not a new requirement). Table anchors are necessary because telecom requirement docs frequently define requirements through cross-reference tables; the leading-id body anchor serves corpora whose requirements are flat body paragraphs under non-requirement section headings. All detected against the same `requirement_id.pattern` regex from the profile, so adding a new MNO still requires no parser code change. Paragraph anchors win on duplicate `req_id`s to keep precedence deterministic.
...
`parse_debug.py`
- `_GLOSSARY_COL0_ACRONYM_RE` — constant — internal
- `_format_row` — function — internal
- `_glossary_position_score` — function — internal — Glossaries can sit anywhere — typically Acronyms/Definitions
- `_load_ir` — function — internal
- `_load_profile` — function — internal — Load the profile through the same substitution chain the parse
- `_looks_like_prose` — function — internal
- `_trunc` — function — internal
- `cmd_glossary` — function — pub
- `cmd_references` — function — pub — Diagnostic preview for the four reference-detection paths:
- `cmd_revhist` — function — pub
- `logger` — constant — pub
- `main` — function — pub
...
- `TocPairMissEntry` — dataclass — pub — A single body heading that didn't pair against the TOC index.
...
`parse_summary.py`
- `CorpusSummary` — dataclass — pub — Top-of-file rollup. Read first by the UI to highlight the
  - `load_json` — classmethod — pub
  - `save_json` — method — pub
  - `to_dict` — method — pub
- `DocSummary` — dataclass — pub — One row per parsed doc — the unit the Summary tab renders.
- `GlossaryMatch` — dataclass — pub — Evidence of a successful glossary / definitions detection.
- `RevhistMatch` — dataclass — pub — Evidence of a successful revhist detection.
- `build_corpus_summary` — function — pub — Aggregate per-doc summaries into a CorpusSummary.
...
- `GenericStructuralParser` — class — pub — Profile-driven structural parser for requirement documents.
  ...
  - `_apply_content_end_marker` — method — internal — Truncate each requirement's text at ``profile.content_end_marker``
  ...
  - `_build_parse_summary` — method — internal — Build the per-doc summary record for the corpus-level
  ...
  - `_classify_heading_docx_styles` — method — internal — ``docx_styles`` classification path — style → depth, TOC → section_number.
  - `_classify_zone` — method — internal — Classify a section into a document zone using profile rules.
  - `_create_leading_id_req` — method — internal — Append a Requirement anchored by a leading req_id in a body block.
  - `_create_table_anchored_req` — method — internal — Append a Requirement node anchored by a table row.
  - `_drop_excluded_sections` — method — internal — Drop sections whose *title* matches ``profile.exclude_section_pattern``
  - `_drop_glossary_subtree` — method — internal — Remove the glossary section and all of its descendants from
  - `_drop_section_subtree` — staticmethod — internal — Return ``sections`` with ``section_num`` + its descendants removed.
  ...
  - `_extract_toc_index` — method — internal — Style-driven TOC pre-pass.
  - `_find_content_start_index` — method — internal — Block index of the first real (font-confirmed) heading whose
  - `_find_req_ids` — method — internal — Find all req_id patterns in `text` and canonicalize each.
  - `_find_revhist_block_indices` — method — internal — Return the block indices that constitute the revision-history
  - `_glossary_section_range` — method — internal — Return (block_start, block_end, page_start, page_end) for the
  - `_heading_req_id` — method — internal — Extract a heading-anchored req_id when the profile opts in.
  - `_heading_title_text` — method — internal — Heading title text per the runs-over-text invariant.
  - `_identify_toc_pages` — method — internal — Return the set of page numbers classified as TOC pages (FR-34).
  - `_is_heading_font` — method — internal — True when a block's font looks like a real heading rather than a
  - `_is_req_id_block` — method — internal — Check if a block is a standalone requirement ID (small font).
  - `_is_requirement` — method — internal — True when `req_id` denotes an actual requirement (not a structural
  - `_link_parents` — method — internal — Build parent-child relationships and hierarchy paths.
  - `_log_format_error` — method — internal — Emit a structured WARN for a likely document formatting error.
  - `_looks_like_definition_column_header` — method — internal — True when (h0, h1) looks like the canonical column-header
  - `_propagate_hierarchy_to_table_reqs` — method — internal — Copy parent's hierarchy_path to section_number-less Requirements.
  - `_score_revhist_table` — method — internal — Score a TABLE block against the revhist signal mix.
  - `_split_reference_entry` — staticmethod — internal — Split a reference entry's content into (spec, title).
  - `_toc_lookup` — method — internal — Resolve a body heading against the TOC index.
...
- `TocEntry` — dataclass — pub — A single TOC line, parsed from a ``toc N``-styled paragraph.
- `TocPairMiss` — dataclass — pub — A body heading that had no matching TOC entry.
- `_DOCX_HEADING_STYLE_RE` — constant — internal
- `_GLOSSARY_LABEL_KEYWORDS` — constant — internal
- `_GLOSSARY_LABEL_MIN_DENSITY` — constant — internal
- `_GLOSSARY_LABEL_STOPWORDS` — constant — internal
- `_GLOSSARY_LABEL_TOKEN_SPLIT_RE` — constant — internal
- `_HEADING_MAX_LEN` — constant — internal
- `_REQ_ID_WHITESPACE_RE` — constant — internal
- `_SECTION_NUM_RE` — constant — internal
- `_TITLE_NORM_RE` — constant — internal
- `_WORD_TOC_STYLE_RE` — constant — internal
- `_canonicalize_req_id` — function — internal — Normalize whitespace in a matched req_id to underscores.
- `_glossary_label_density` — function — internal — Return ``(keyword_hits, meaningful_token_count, density, hit_tokens)``
- `_normalize_title` — function — internal
- `build_context_string` — function — pub — Assemble a requirement's enclosing-section context (D-DRAFT-5).
- `logger` — constant — pub
- `render_table_markdown` — function — pub — Render a parsed table as GitHub-flavored markdown.
