...
- Golden eval (strand golden-eval; operator runbook: [docs/golden-eval-guide.md](../../../docs/golden-eval-guide.md)):
  ...
  - CLI: `golden_cli.main` — batch entrypoint (stack URL(s), stage selection, env_dir); runnable standalone as a build step or as a final pipeline stage. `--mno` filters the run to samples tagged with that carrier (filter applies BEFORE the stamp freezes the eval set, so filtered runs key apart from full runs)
